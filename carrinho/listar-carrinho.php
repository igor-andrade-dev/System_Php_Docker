...
if($sessao != ""){
  $res = $pdo->query("SELECT * from carrinho where sessao = '$sessao' and id_venda = 0 order by id asc");
}else{
   $res = $pdo->query("SELECT * from carrinho where (id_usuario = '$id_usuario') and id_venda = 0 order by id asc");
}

if($id_usuario == 0 and $sessao == ""){
  $res = $pdo->query("SELECT * from carrinho where id = 0");
}
...
if($total_prod_carac > 0){
  ...
  $valor_item_carac = $res4[$i2]['adicional'];
  if($valor_item_carac > 0){
    $valor_item_carac = '<span class="text-danger"><small> (+ R$ '.$valor_item_carac.')</small></span>';
  }else{
    $valor_item_carac = '';
  }
  ...
  $nome_carac = $res1[0]['nome'];
    ...
    echo '<small><span class="mr-2"><i class="mr-1 fa fa-check text-info"></i>'.$nome_carac.' : '.$nome_item_carac.' '.$valor_item_carac.'</span></small><br>';
...
<script type="text/javascript">
  ...
  $("#total_itens_cabecalho").text(itens);
  $("#total_valor_cabecalho").text(total);

   $("#total_itens_cabecalho_menu").text(itens);
  $("#total_valor_cabecalho_menu").text(total);
